#MAIN THESIS SCRIPT
#Script that trains a series of Long Short Term Memory deep learning models in tensorflow for binary classification of long sequence (a book review)
#Takes hyper parameter settings from an auxilliary JSON file and runs all models to 5000 steps, saving training data every 50 steps and models every 1000 steps

#Code adapted from https://www.oreilly.com/learning/perform-sentiment-analysis-with-lstms-using-tensorflow
#Word2Vec and GloVe lookup functions taken from https://github.com/cahya-wirawan/text-classification/blob/master/textcnn_datahelpers.py
...
settings2 = json.load(open('fullsettings.json'))
...
for settings in settings2:
    ...
    lstmUnits = settings2[settings].get("lstmUnits",32)
    lstmUnits = int(lstmUnits)

    dropout = settings2[settings].get("output_keep_prob", 0.5)
    dropout = float(dropout)

    vectorpath = settings2[settings].get("vectorpath","C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\GoogleNews-vectors-negative300.bin")

    optimizer = settings2[settings].get("optimizer", 'Adam')

    textprocess = settings2[settings].get("textprocess", None)
    ...
    if vectorpath == "C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\GoogleNews-vectors-negative300.bin":
    ...
    if vectorpath == 'C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\glove.6B.50d.txt':
        numDimensions = 50
    else:
        numDimensions = 300
    ...
    if vectorpath == 'C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\glove.6B.50d.txt':
        ...
        wordVectors = load_embedding_vectors_glove(vocabulary, vectorpath, 50)

    elif vectorpath == 'C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\glove.6B.300d.txt':

        wordVectors = load_embedding_vectors_glove(vocabulary, vectorpath, 300)

    elif vectorpath == "C:\\Users\\Clembo\\Desktop\\Thesis\\Data\\GoogleNews-vectors-negative300.bin":
